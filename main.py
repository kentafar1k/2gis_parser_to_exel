...
def main():
    search_query = 'Массаж'
    url = f"https://2gis.ru/vologda/search/{search_query}"
    ...
    text = get_element_text(driver, pathes.items_count)
    if text.isdigit():
        count_all_items = int(text)
    else:
        print(f"Ошибка: не удалось преобразовать '{text}' в число.")
        driver.quit()
        return
    ...
    for _ in range(pages):
        ...
        for item in range(1, count_items + 1):
            if main_block.find_element(By.XPATH, f'div[{item}]').getAttribute('class'):
                continue
